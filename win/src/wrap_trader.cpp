...
std::atomic_int WrapTrader::s_uuid;
...
Handle<Value> WrapTrader::Connect(const Arguments& args) {
	...
	if (!args[4]->IsUndefined() && args[4]->IsFunction()) {
		...
		logger_cout(_head.append(" uuid is ").append(std::to_string(uuid)).c_str());
	}
	...
	logger_cout(log.append(" ").append((std::string)*addrAscii).append("|").append((std::string)*pathAscii).append("|").append(std::to_string(publicTopicType)).append("|").append(std::to_string(privateTopicType)).c_str());
...
Handle<Value> WrapTrader::ReqOrderInsert(const Arguments& args) {
	...
	logger_cout(log.append(" ").
		...
		append(std::to_string(limitPrice)).append("|").
		append(std::to_string(volumeTotalOriginal)).append("|").
		append((std::string)*timeCondition_).append("|").
		append((std::string)*volumeCondition_).append("|").
		append(std::to_string(minVolume)).append("|").
		append((std::string)*contingentCondition_).append("|").
		append((std::string)*forceCloseReason_).append("|").
		append(std::to_string(isAutoSuspend)).append("|").
		append(std::to_string(userForceClose)).append("|").c_str());
...
Handle<Value> WrapTrader::ReqOrderAction(const Arguments& args) {
	...
	logger_cout(log.append(" ").
		...
		append((std::string)*instrunmentId_).append("|").
		append((std::string)*orderRef_).append("|").
		append(std::to_string(frontId)).append("|").
		append(std::to_string(sessionId)).append("|").
		append((std::string)*exchangeID_).append("|").
		append((std::string)*orderSysID_).append("|").
		append(std::to_string(actionFlag)).append("|").c_str());
...
Handle<Value> WrapTrader::ReqQryInstrumentMarginRate(const Arguments& args) {
	...
	logger_cout(log.append(" ").
		...
		append(std::to_string(hedgeFlag)).append("|").c_str());	 
...
void WrapTrader::FunRtnCallback(int result, void* baton) {
	HandleScope scope;
	auto tmp = static_cast<LookupCtpApiBaton*>(baton);	 
	if (tmp->uuid != -1) {
		logger_cout(std::to_string(tmp->uuid).append(":uuid,").append("function return callback").c_str());
